...
// import faq from '../../../contents/faq-page/content.yml';
...
export const MaterialiRisorsePage = () => {
  ...
  function countInitQuestions() {
    let count = 0;
    content.questions.forEach((element) => {
  ...
  const handleChange = (event) => {
    ...
    if (value.length >= 3) {
      ...
      if (isMobile) {
        filterId !== 'all' ? setQuestions(getQuestionsMobile(content.questions)) : setQuestions(content.questions);
      } else {
  ...
  useEffect(() => {
    ...
    const chipHandler = (event) => {
      const questionsModel = JSON.parse(JSON.stringify(content.questions));
      let currentTarget;
  ...
  return (
    <>
      ...
      <HeroSupport
        ...
        isFaq={true}
        breadCrumbLabel="Materiali e risorse"
        isMatResource={true}
      />
      <div className="docs py-4 py-md-5">
        <Container className="px-3">
          <h2 id="question-section" className="sr-only">
            Sezione materiali e risorse
          </h2>
          <Row>
            <Col lg={9} className="offset-lg-3 px-lg-3">
              <div role="search" className={classes.inputContainer} aria-label="Nei materiali e risorse">
                <div id="searchbox-desk" className="sr-only">
                  Ad ogni digitazione il numero di argomenti presenti in pagina verrà aggiornato.
                </div>
                <Input
                  className={inputValue.length > 0 ? '' : classes.inputWrap}
                  type="text"
                  label="Cerca nei materiali e risorse"
                  id="faq-search"
                  role="searchbox"
